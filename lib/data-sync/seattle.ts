...
const KNOWN_IDS = [
  ...
  'ez4a-iug7',  // Code Complaints and Violations — verified 2026-05-02 (239,824 rows)
  '8s4s-3hc9',  // Code Complaints and Violations (map view, same dataset)
  'uyyd-8gak',  // legacy: Real Property data
  'kzjm-xkqj',  // legacy: Active Rental Registrations
  'p8p5-ust6',  // legacy: Building Inspections
  'k2p3-jnbc',
  'v74d-5n7a',
  'mags-97de',
...
export async function syncSeattle(supabase: SupabaseClient): Promise<SyncResult> {
  ...
  while (true) {
    ...
    for (const row of rows) {
      // ez4a-iug7 (current dataset) ships `originaladdress1`, no underscores.
      // Older datasets used `original_address_1` / `site_address`.
      const addr = row.originaladdress1 ?? row.property_address ?? row.address ?? row.original_address_1 ?? row.site_address ?? ''
    ...
    for (const row of rows) {
      const sourceId = String(row.recordnum ?? row.case_number ?? row.permit_number ?? row.application_permit_number ?? row.object_id ?? row.id ?? '')
      if (!sourceId) { result.skipped++; continue }
      const addr = row.originaladdress1 ?? row.property_address ?? row.address ?? row.original_address_1 ?? ''
      const propertyId = addr ? (propIdMap.get(normalizeAddress(addr)) ?? null) : null
      const desc = row.description ?? row.recordtypedesc ?? row.recordtypemapped ?? row.complaint_type ?? row.permit_type ?? row.action_type ?? null
      const status = (row.statuscurrent ?? row.status ?? '').toLowerCase()
      toInsert.push({
        ...
        status: status.includes('close') ? 'closed' : 'open',
        filed_date: (row.opendate ?? row.original_file_date ?? row.application_date ?? row.issue_date ?? row.date)
          ? new Date(row.opendate ?? row.original_file_date ?? row.application_date ?? row.issue_date ?? row.date).toISOString().split('T')[0]
          : null,
        raw_data: row,
      })
    }
