/**
 * Cuyahoga County (Cleveland) Property Assessment
 * Source: Cuyahoga County Open Data
 * Covers Cleveland + all Cuyahoga County cities (Lakewood, Parma, Euclid, etc.)
 */
...
const KNOWN_IDS = [
  process.env.CUYAHOGA_DATASET,
  'rpkj-w3sm',  // Cuyahoga County property data
  'bx67-gwi5',  // Property assessment
  '7wfb-34y2',  // Parcel ownership
].filter(Boolean) as string[]

const DOMAINS = ['data.cuyahogacounty.us', 'opendata.cuyahogacounty.us']
...
export async function syncCuyahogaAssessor(supabase: SupabaseClient): Promise<SyncResult> {
  const result: SyncResult = { added: 0, updated: 0, skipped: 0, errors: [] }

  const endpoint = await findWorkingEndpoint()
  if (!endpoint) {
    result.errors.push('No working Cuyahoga County assessor endpoint. Set CUYAHOGA_DATASET env var from data.cuyahogacounty.us.')
    return result
  }
  ...
  while (true) {
    ...
    for (const row of rows) {
      try {
        ...
        const address = (row.address ?? row.property_address ?? row.situs_address ?? row.parcel_address ?? '').trim()
        if (!address) { result.skipped++; continue }

        const city = (row.city ?? row.municipality ?? row.muni ?? 'Cleveland').trim()
        ...
        if (!landlordId) {
          ...
          if (existing) {
            ...
            const baseSlug = slugify(`${ownerName}-cleveland`, { lower: true, strict: true })
...
const GOV_KEYWORDS = [
  'cuyahoga county', 'city of cleveland', 'cleveland housing authority',
