-- ============================================================
-- VettRentals — SAFE TO RE-RUN (uses IF NOT EXISTS throughout)
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  email_watchlist       BOOLEAN NOT NULL DEFAULT true,
...
CREATE INDEX IF NOT EXISTS idx_landlords_slug ON public.landlords(slug);
CREATE INDEX IF NOT EXISTS idx_landlords_city_state ON public.landlords(city, state_abbr);
CREATE INDEX IF NOT EXISTS idx_landlords_claimed ON public.landlords(is_claimed);
CREATE INDEX IF NOT EXISTS idx_landlords_verified ON public.landlords(is_verified);
...
CREATE INDEX IF NOT EXISTS idx_properties_city_state ON public.properties(city, state_abbr);
CREATE INDEX IF NOT EXISTS idx_properties_zip ON public.properties(zip);
CREATE UNIQUE INDEX IF NOT EXISTS idx_properties_normalized ON public.properties(address_normalized, city, state_abbr)
...
CREATE INDEX IF NOT EXISTS idx_reviews_landlord ON public.reviews(landlord_id);
...
CREATE INDEX IF NOT EXISTS idx_records_property ON public.public_records(property_id);
CREATE INDEX IF NOT EXISTS idx_records_landlord ON public.public_records(landlord_id);
CREATE INDEX IF NOT EXISTS idx_records_type ON public.public_records(record_type);
CREATE INDEX IF NOT EXISTS idx_records_source ON public.public_records(source, source_id);
CREATE INDEX IF NOT EXISTS idx_records_severity ON public.public_records(severity);
CREATE INDEX IF NOT EXISTS idx_records_status ON public.public_records(status);
CREATE INDEX IF NOT EXISTS idx_records_filed ON public.public_records(filed_date DESC);
CREATE UNIQUE INDEX IF NOT EXISTS idx_records_source_dedup ON public.public_records(source, source_id)
  WHERE source_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS public.unlinked_records (
  id         UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  source     TEXT NOT NULL,
  source_id  TEXT,
...
CREATE INDEX IF NOT EXISTS idx_flags_review ON public.review_flags(review_id);
CREATE INDEX IF NOT EXISTS idx_flags_status ON public.review_flags(status);
...
CREATE INDEX IF NOT EXISTS idx_disputes_status ON public.record_disputes(status);

CREATE TABLE IF NOT EXISTS public.landlord_claims (
  id                UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  landlord_id       UUID REFERENCES public.landlords(id) ON DELETE CASCADE,
  claimed_by        UUID REFERENCES public.profiles(id),
...
CREATE INDEX IF NOT EXISTS idx_claims_status ON public.landlord_claims(status);
CREATE INDEX IF NOT EXISTS idx_claims_landlord ON public.landlord_claims(landlord_id);
...
CREATE INDEX IF NOT EXISTS idx_submissions_status ON public.landlord_submissions(status, created_at);
...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='landlords_select_all' AND tablename='landlords') THEN
    CREATE POLICY "landlords_select_all" ON public.landlords FOR SELECT USING (TRUE);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='landlords_admin_write' AND tablename='landlords') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='properties_admin_write' AND tablename='properties') THEN
    CREATE POLICY "properties_admin_write" ON public.properties FOR ALL USING (is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='reviews_select_approved' AND tablename='reviews') THEN
    CREATE POLICY "reviews_select_approved" ON public.reviews FOR SELECT USING (status = 'approved' OR reviewer_id = auth.uid() OR is_admin());
  END IF;
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='reviews_update_own' AND tablename='reviews') THEN
    CREATE POLICY "reviews_update_own" ON public.reviews FOR UPDATE USING (reviewer_id = auth.uid() OR is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='records_select_all' AND tablename='public_records') THEN
    CREATE POLICY "records_select_all" ON public.public_records FOR SELECT USING (TRUE);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='records_admin_write' AND tablename='public_records') THEN
    CREATE POLICY "records_admin_write" ON public.public_records FOR ALL USING (is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='disputes_select' AND tablename='record_disputes') THEN
    CREATE POLICY "disputes_select" ON public.record_disputes FOR SELECT USING (disputed_by = auth.uid() OR is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='disputes_insert' AND tablename='record_disputes') THEN
    CREATE POLICY "disputes_insert" ON public.record_disputes FOR INSERT WITH CHECK (disputed_by = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='disputes_update_admin' AND tablename='record_disputes') THEN
    CREATE POLICY "disputes_update_admin" ON public.record_disputes FOR UPDATE USING (is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='claims_select_own' AND tablename='landlord_claims') THEN
    CREATE POLICY "claims_select_own" ON public.landlord_claims FOR SELECT USING (claimed_by = auth.uid() OR is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='claims_insert_own' AND tablename='landlord_claims') THEN
    CREATE POLICY "claims_insert_own" ON public.landlord_claims FOR INSERT WITH CHECK (claimed_by = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='claims_update_admin' AND tablename='landlord_claims') THEN
    CREATE POLICY "claims_update_admin" ON public.landlord_claims FOR UPDATE USING (is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='watchlist_own' AND tablename='watchlist') THEN
    CREATE POLICY "watchlist_own" ON public.watchlist FOR ALL USING (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='flags_insert' AND tablename='review_flags') THEN
    CREATE POLICY "flags_insert" ON public.review_flags FOR INSERT WITH CHECK (flagged_by = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='flags_admin' AND tablename='review_flags') THEN
    CREATE POLICY "flags_admin" ON public.review_flags FOR SELECT USING (is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='helpful_own' AND tablename='review_helpful') THEN
    CREATE POLICY "helpful_own" ON public.review_helpful FOR ALL USING (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='helpful_select' AND tablename='review_helpful') THEN
    CREATE POLICY "helpful_select" ON public.review_helpful FOR SELECT USING (TRUE);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='sync_log_admin' AND tablename='sync_log') THEN
    CREATE POLICY "sync_log_admin" ON public.sync_log FOR ALL USING (is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='submissions_own' AND tablename='landlord_submissions') THEN
    CREATE POLICY "submissions_own" ON public.landlord_submissions FOR SELECT USING (submitted_by = auth.uid() OR is_admin());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='submissions_insert' AND tablename='landlord_submissions') THEN
    CREATE POLICY "submissions_insert" ON public.landlord_submissions FOR INSERT WITH CHECK (submitted_by = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname='submissions_admin' AND tablename='landlord_submissions') THEN
    CREATE POLICY "submissions_admin" ON public.landlord_submissions FOR UPDATE USING (is_admin());
  END IF;
END $$;

-- ─── FULL TEXT SEARCH ────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_landlords_fts ON public.landlords
  USING GIN (to_tsvector('english', coalesce(display_name,'') || ' ' || coalesce(business_name,'') || ' ' || coalesce(city,'')));
CREATE INDEX IF NOT EXISTS idx_properties_fts ON public.properties
...
RETURNS TABLE (
  ...
  SELECT l.id, 'landlord'::TEXT, l.display_name, COALESCE(l.business_name,''),
    l.slug, l.avg_rating, l.review_count, l.city, l.state_abbr,
    ts_rank(to_tsvector('english', l.display_name || ' ' || COALESCE(l.business_name,'') || ' ' || COALESCE(l.city,'')), plainto_tsquery('english', query)) AS rank,
    l.is_claimed
  FROM public.landlords l
  WHERE to_tsvector('english', l.display_name || ' ' || COALESCE(l.business_name,'') || ' ' || COALESCE(l.city,'')) @@ plainto_tsquery('english', query)
     OR l.display_name ILIKE '%' || query || '%'
  UNION ALL
  SELECT p.id, 'property'::TEXT, p.address_line1, p.city || ', ' || p.state_abbr,
    p.id::TEXT, p.avg_rating, p.review_count, p.city, p.state_abbr,
    ts_rank(to_tsvector('english', p.address_line1 || ' ' || p.city), plainto_tsquery('english', query)) AS rank,
    FALSE
...
-- ─── TRIGGERS & FUNCTIONS ────────────────────────────────────
CREATE OR REPLACE FUNCTION public.compute_landlord_grade(avg_r NUMERIC, open_violations INT, evictions INT)
RETURNS TEXT LANGUAGE plpgsql AS $$
DECLARE score NUMERIC;
BEGIN
  score := COALESCE(avg_r,0)*20 - COALESCE(open_violations,0)*5 - COALESCE(evictions,0)*10;
  score := GREATEST(0, LEAST(100, score));
  IF score >= 85 THEN RETURN 'A'; ELSIF score >= 70 THEN RETURN 'B';
  ELSIF score >= 55 THEN RETURN 'C'; ELSIF score >= 40 THEN RETURN 'D'; ELSE RETURN 'F'; END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.update_landlord_grade() RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN NEW.grade := public.compute_landlord_grade(NEW.avg_rating, NEW.open_violation_count, NEW.eviction_count); RETURN NEW; END;
$$;
DROP TRIGGER IF EXISTS trg_landlord_grade ON public.landlords;
CREATE TRIGGER trg_landlord_grade BEFORE INSERT OR UPDATE ON public.landlords FOR EACH ROW EXECUTE FUNCTION public.update_landlord_grade();

CREATE OR REPLACE FUNCTION public.generate_landlord_slug(name TEXT, city TEXT) RETURNS TEXT LANGUAGE plpgsql AS $$
DECLARE base_slug TEXT; candidate TEXT; counter INT := 1;
BEGIN
  base_slug := lower(regexp_replace(regexp_replace(unaccent(name||'-'||COALESCE(city,'')), '[^a-zA-Z0-9\s-]','','g'),'\s+','-','g'));
  base_slug := trim(both '-' from left(base_slug, 60));
  candidate := base_slug;
...
CREATE OR REPLACE FUNCTION public.set_landlord_slug() RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN IF NEW.slug IS NULL OR NEW.slug='' THEN NEW.slug := public.generate_landlord_slug(NEW.display_name, NEW.city); END IF; RETURN NEW; END;
$$;
DROP TRIGGER IF EXISTS trg_landlord_slug ON public.landlords;
CREATE TRIGGER trg_landlord_slug BEFORE INSERT ON public.landlords FOR EACH ROW EXECUTE FUNCTION public.set_landlord_slug();

CREATE OR REPLACE FUNCTION public.update_landlord_stats() RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF (TG_OP='INSERT' AND NEW.status='approved' AND NEW.landlord_id IS NOT NULL) OR
     (TG_OP='UPDATE' AND OLD.status!=NEW.status AND NEW.landlord_id IS NOT NULL) THEN
...
CREATE TRIGGER trg_landlord_stats AFTER INSERT OR UPDATE ON public.reviews FOR EACH ROW EXECUTE FUNCTION public.update_landlord_stats();

CREATE OR REPLACE FUNCTION public.update_violation_counts() RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE l_id UUID;
BEGIN
  l_id := COALESCE(NEW.landlord_id, OLD.landlord_id);
  IF l_id IS NOT NULL THEN
    UPDATE public.landlords SET
      open_violation_count=(SELECT COUNT(*) FROM public.public_records WHERE landlord_id=l_id AND status NOT IN ('closed','dismissed') AND record_type NOT IN ('court_case','court_listener','eviction','eviction_filing','lsc_eviction')),
...
CREATE TRIGGER trg_violation_counts AFTER INSERT OR UPDATE OR DELETE ON public.public_records FOR EACH ROW EXECUTE FUNCTION public.update_violation_counts();
...
DROP TRIGGER IF EXISTS trg_landlords_updated_at ON public.landlords;
...
RETURNS TABLE(id UUID, display_name TEXT, similarity FLOAT) LANGUAGE sql STABLE SECURITY DEFINER AS $$
...
-- ============================================================
-- Done! All tables, indexes, RLS policies, triggers,
