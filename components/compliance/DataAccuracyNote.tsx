...
export function DataAccuracyNote({ source, lastSynced, recordId }: DataAccuracyNoteProps) {
  ...
  return (
    <span className="flex items-start gap-1 text-xs text-slate-400 mt-1">
      ...
      <span>
        Source: {sourceLabel}. Last updated: {lastSynced ? <time dateTime={lastSynced}>{formatDate(lastSynced)}</time> : formatDate(lastSynced)}. Data may not reflect recent changes.{' '}
        {recordId && (
