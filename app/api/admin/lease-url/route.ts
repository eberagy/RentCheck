...
export async function GET(req: NextRequest) {
  ...
  if (error || !data) {
    // Storage signed-URL failures used to drop straight to a generic
    // 500 with no breadcrumb — invisible to the team if the bucket
    // policy or service role expires.
    if (error) captureException(error, { where: 'admin/lease-url:storage', reviewId: reviewId ?? undefined })
